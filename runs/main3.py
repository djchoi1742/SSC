...
network_config.add_argument('--num_epoch', type=int, dest='num_epoch', default=160)  # infinite loop
...
network_config.add_argument('--max_keep', type=int, dest='max_keep', default=3)  # only use training
network_config.add_argument('--num_weight', type=int, dest='num_weight', default=3)  # only use validation
network_config.add_argument('--image_size', type=int, dest='image_size', default=280)  # prev: 280
...
network_config.add_argument('--last_kp', type=int, dest='last_kp', default=1)
...
is_flip = get_info(excel_path, data_name+'_val', 'IS_FLIP')
...
if config.train:
    ...
    growth_k = model_y.get_train_info(result_path, 'GROWTH_K')
    block_rep = model_y.get_train_info(result_path, 'BLOCK_REP')
    dense_type = model_y.get_train_info(result_path, 'DENSE_TYPE')


model = getattr(model_y, infer_name)(trainable=config.train, growth_k=growth_k,
                                     block_rep=block_rep, dense_type=dense_type,
                                     image_size=config.image_size, image_height=image_height,
...
def training():
    info_log = {
        ...
        'LAST_KP': config.last_kp,
        'BLOCK_REP': config.block_rep,
        'DENSE_TYPE': config.dense_type,
        'K_P': config.k_p
    }
        ...
        while True:  # increment of training epochs
            ...
            if current_epoch % 1 == 0:
                ...
                if current_epoch < config.max_keep + 1:
                    ...
                    auc_csv.loc[current_step, 'WEIGHT_PATH'] = os.path.join(log_path, 'model.ckpt-'+str(current_step))
...
def validation():
    ...
    ppt_name = os.path.join(ppt_path, '_'.join([config.exp_name, config.model_name, data_name, trial_serial_str,
