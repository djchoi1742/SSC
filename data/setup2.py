...
setup_config.add_argument('--exp_name', type=str, dest='exp_name', default='exp151')
setup_config.add_argument('--data_name', type=str, dest='data_name', default='test23', help='npy name')
setup_config.add_argument('--train_excel', type=str, dest='train_excel', default='exp151_test23.xlsx')
setup_config.add_argument('--val_excel', type=str, dest='val_excel', default='exp151_test23.xlsx')
...
setup_config.add_argument('--w_radius', type=int, dest='w_radius', default=28)  # prev: 28
setup_config.add_argument('--h_radius', type=int, dest='h_radius', default=21)  # prev: 21
setup_config.add_argument('--image_size', type=int, dest='image_size', default=280)  # prev: 280, image width
setup_config.add_argument('--is_flip', type=lambda x: x.title() in str(True), dest='is_flip', default=True)
    ...
    def add_value(excel, view_type, w_radius, h_radius):
        ...
        excel[view_name + '_COORD'] = excel.apply(
            lambda row: calculate_crop_coord(center_x=row['LC_COORD_X'], center_y=row['LC_COORD_Y'],
                                             ...
                                             y1_adj=0.8, y2_adj=1.2), axis=1)  # exp111: 1.0, 1.0
...
class DataSettingV1:
    def __init__(self, data_dir, batch_size, only_val, image_size, image_height, replicate=False,
                 rep_label=0, rep_count=2, aug_param=20, is_flip=True, **kwargs):
    ...
    class SubDataSetting:
        def __init__(self, files_n_labels, num_epochs=1, batch_size=1,
            ...
            data_set = tf.data.Dataset.from_tensor_slices(tensors=(self.file5,
            ...
            def dcm_read_by_ftn(file5, label5, id, clv, image_size, is_flip, augmentation):
                def each_read(filename, label, is_flip, augmentation):
            ...
            data_set = data_set.map(num_parallel_calls=8,
                                    ...
                                    tuple(tf.py_func(func=dcm_read_by_ftn,
                                                     inp=[file5, label5, id, clv,
                                                          image_size, is_flip, augmentation],
...
def test_data_loader(data_set):
    ...
    with tf.Session() as sess:
        ...
        while count < num_iter:
            img, lbl, name, age, tm, dm, vas, side = sess.run(data_set.val.next_batch)

            blank_slide_layout = prs.slide_layouts[6]
            slide = prs.slides.add_slide(blank_slide_layout)

            show_images(img, name, side, num_rows=6, num_cols=10, fig_size=(10*2, 6*2))
...
def show_images(images, names, sides, num_rows=6, num_cols=10, fig_size=(10*2, 6*2)):
    ...
    for j in range(num_figs):
        ...
        plt_name = '_'.join([str(img_name.decode('utf-8')), side_value])
